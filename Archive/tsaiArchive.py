...
sx=1 #Valeur = 1
dx=3.45e-6 #Center to center distance between adjacent sensor element in X
dy=3.45e-6 #Center to center distance between adjacent sensor element in Y
Ncx=2464   #Number of sensor element in X direction
Nfx=2464  #Number of pixel in a line as sampled by the computer ????
dxx=dx*Ncx/Nfx
# iii)
#Row ans column number of center of computer frame memory
Cx=imageShape[0]/2;Cy=imageShape[1]/2;
C = np.array([Cx, Cy])
# iv) Compute Xdi Ydi
Xd=sx**(-1)*dxx*(Xf-Cx)
Yd=dy*(Yf-Cy)
...
# 1)Compute |Ty|
normTy = 1/np.sqrt(a5**2 + a6**2 + a7**2)

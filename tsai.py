...
pointsPath = "data/serie_1/points/points.txt"
outputPath = "data/serie_1/tsai/"

def calibrate(imageShape,pointsPath,outputPath):
    ...
    # ii)
    dx=1e-6 #Center to center distance between adjacent sensor element in X
    dy=1e-6 #Center to center distance between adjacent sensor element in Y
    ...
    C=(Cx,Cy) #Row ans column number of center of computer frame memory
    # iv) Compute Xdi Ydi
    Xd=sx**(-1)*dxx*(Xf-Cx)
    Yd=dy*(Yf-Cy)
    ...
    #b) compute the exact solution for f, Tx, k1
    return f, Tx
